...
public class Control {
	...
	Control(UI ui){
		...
		_patternTest = new UIFormTest(){
			public boolean run(String input){
				return (input.equalsIgnoreCase("simple") || input.equalsIgnoreCase("alternating"));
			}
		};
	...
	private void addStart(int state) {
		...
		menu.add("Run Simulation", new UIMenuAction(){
			public void run(){
				//TODO change to creating simulation!
				_ui.displayMessage("Not yet ready");
				AnimatorBuilder build = new SwingAnimatorBuilder();
				SimpleModel model = new SimpleModel(build);
